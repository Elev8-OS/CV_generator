...
const DICT = {
  ...
  profile: {
    ...
    experienceSupplementNoNew: { de: "Keine neuen Positionen gefunden.", fr: "Aucun nouveau poste trouvé.", en: "No new positions found." },
...
function t(lang, keyPath, vars) {
  ...
  if (vars) {
    Object.keys(vars).forEach((k) => {
      value = value.replace(new RegExp(`{{\\s*${k}\\s*}}`, "g"), vars[k] == null ? "" : String(vars[k]));
    });
  }
  return value;
}
