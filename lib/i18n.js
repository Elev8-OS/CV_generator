...
const DICT = {
  ...
  theme: {
    ...
    subtitle: {
      ...
      fr: "Cette couleur est utilisée dans l'outil, sur la page de candidature numérique et dans les PDF générés (CV, lettre de motivation). Une modification s'applique aussi rétrospectivement aux candidatures déjà créées.",
...
function t(lang, keyPath, vars) {
  ...
  if (vars) {
    Object.keys(vars).forEach((k) => {
      value = value.replace(new RegExp(String.fromCharCode(92) + String.fromCharCode(92) + "s*" + "${k}".replace("${k}", k) + String.fromCharCode(92) + String.fromCharCode(92) + "s*", "g"), vars[k] == null ? "" : String(vars[k]));
    });
  }
  return value;
}
