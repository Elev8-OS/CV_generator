...
async function generateApplication({ profile, jobPostingText, jobUrl, libraryDocs = [], language = "de" }) {
  ...
  const response = await client.messages.create({
    model: MODEL,
    // Was 4000 — too tight in practice: a full application (summary, email,
    // formal cover letter, 4-6 selectedStrengths, experienceHighlights across
    // every experience id, digitalPageIntro, whyThisRole) in verbose German
    // can genuinely need more, and fitScore/fitScoreReasoning are declared
    // LAST in TOOL_SCHEMA — so when the response got cut off near the token
    // limit, those two trailing fields were the ones silently missing from
    // toolUse.input while everything else looked fine. That's what surfaced
    // as "fit score not shown" on an otherwise normal-looking new
    // application. 8000 matches the headroom already used elsewhere in this
    // file for less critical extraction calls.
    max_tokens: 8000,
  ...
  if (response.stop_reason === "max_tokens") {
    // Rather than silently returning a partial tool input (which could be
    // missing arbitrary trailing fields, not just fitScore), fail loudly so
    // this is visible instead of shipping a subtly-broken application.
    const err = new Error(
      "Die Generierung wurde wegen der Antwortlänge abgeschnitten, bevor sie fertig war. Bitte erneut versuchen."
    );
    err.code = "TRUNCATED";
    throw err;
  }
  ...
  if (typeof toolUse.input.fitScore !== "number") {
    // Marking a field "required" in the tool schema is only guidance to the
    // model, not an API-enforced contract -- it has been observed to omit
    // fitScore anyway despite the response otherwise looking complete. Coerce
    // a numeric-looking string defensively (mirrors coerceArrayField above
    // for the same class of "right value, wrong JSON type" issue) and log
    // the rest so this is traceable instead of silently showing "-" in the
    // dashboard's Fit column.
    const coerced = Number(toolUse.input.fitScore);
    if (Number.isFinite(coerced)) {
      toolUse.input.fitScore = coerced;
    } else {
      console.error("ai.js: generateApplication kam ohne gültigen fitScore zurück:", toolUse.input.fitScore);
      toolUse.input.fitScore = null;
    }
  }
