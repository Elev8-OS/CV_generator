...
function formatDate(d = new Date(), lang = "de") {
  return d.toLocaleDateString(localeFor(lang), { day: "2-digit", month: "long", year: "numeric" });
}
 ...
 */
function buildCoverDocDefinition(profile, generated, { location, qr, signature, lang = "de", accentColor } = {}, level = 0) {
  const l = normalizeLang(lang);
  const theme = resolveTheme(accentColor);
  const p = profile.personal;
  const L = COMPACT_LEVELS[Math.min(level, COMPACT_LEVELS.length - 1)];
  // Defaults to the person's own town from their profile (a formal Swiss
  // letter dateline is conventionally "<Ort>, <Datum>") rather than a
  // hardcoded place — falls back to just the date if no town is on file yet.
  const dateline = location || p.ort || "";
  ...
  const recipientLines = [generated.company, generated.contactName ? generated.contactName : t(l, "pdf.defaultRecipient"), generated.location]
    .filter(Boolean);
  ...
  return {
    ...
    content: [
      ...
      {
        text: dateline ? `${dateline}, ${formatDate(new Date(), l)}` : formatDate(new Date(), l),
        alignment: "right",
        fontSize: L.fontSize - 1,
        color: theme.grey,
        margin: [0, 0, 0, L.dateGap]
      },
      {
        text: generated.emailSubject || t(l, "pdf.defaultSubject", { jobTitle: generated.jobTitle }),
        bold: true,
      ...
      ...paragraphs.map((para, i) => ({ text: para, margin: [0, i === 0 ? 0 : L.paraGap, 0, 0] })),
      {
        text: t(l, "pdf.greeting"),
