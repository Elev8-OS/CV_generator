...
function saveMedia(userId, key, buffer, mimetype, lang = DEFAULT_LANG) {
  if (!MEDIA_KEYS.includes(key)) throw new Error(t(lang, "server.unknownMediaType", { key }));
  const ext = EXT_BY_MIME[mimetype];
  if (!ext) throw new Error(t(lang, "server.invalidImageFormat"));
