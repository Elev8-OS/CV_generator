...
const DEFAULT_PROFILE = {
  ...
  personal: {
    ...
    // AHV-Nr. (Swiss social-security number) — used only for the RAV
    // "Nachweis der persönlichen Arbeitsbemühungen" (form 716.007) PDF, see
    // lib/pdf/rav.js. Left blank even for Raffael's own default profile
    // (nothing in this codebase should hardcode a real AHV number) — filled
    // in via the AHV-Nr. field on /profile.
    ahvNr: "",
  ...
  // Contact details of the person's own RAV/ALV advisor — used only to
  // prefill the "An Berater:in senden" mailto link on the dashboard's RAV
  // card (see lib/pages/indexPage.js / public/app.js), never printed onto
  // the RAV PDF itself (lib/pdf/rav.js stays a close facsimile of the
  // official, advisor-agnostic form). Filled in via the "RAV-Berater:in"
  // card on /profile; blank changes nothing else, the send button just stays
  // hidden until an advisor email is on file.
  rav: {
    advisorName: "",
    advisorEmail: "",
    advisorPhone: "",
...
// Built via String.fromCharCode/RegExp constructor rather than literal
// unicode-escape sequences or literal accented/dash characters in this
// source file -- this
// codebase's GitHub-push pipeline has repeatedly (and unpredictably)
// mangled literal backslash-escape sequences in pushed file content, so
// non-ASCII matching logic is built at runtime from character codes instead,
// making the source file itself immune to that whole class of transcription
// bug (same fix pattern as the bullets join/split in profilePage.js).
const COMBINING_MARKS_RE = new RegExp("[" + String.fromCharCode(0x0300) + "-" + String.fromCharCode(0x036f) + "]", "g");
// Hyphen placed last in the character class so it's a literal (no escaping needed).
const DASH_PUNCTUATION_RE = new RegExp("[.,;:()&/" + String.fromCharCode(0x2013) + String.fromCharCode(0x2014) + "-]", "g");

function normalizeMatchKey(s) {
  return String(s || "")
    .toLowerCase()
    .normalize("NFKD")
    .replace(COMBINING_MARKS_RE, "") // strip combining accents left over by NFKD (e.g. e + accent -> e)
    .replace(DASH_PUNCTUATION_RE, " ") // punctuation incl. hyphen / en-dash / em-dash
...
// --- Chronological ordering for mergeAdditionalExperience -------------------
// Newly-merged entries used to simply be appended after all existing ones,
// regardless of when they actually happened -- uploading a LinkedIn export
// later would land its (possibly CURRENT) positions at the very bottom of
// the list, below decades-old jobs. This parses each entry's "period" string
// well enough to sort the combined list by start date, most recent first, so
// merged entries land in their correct chronological position instead of
// always at the end. Supports the period formats actually produced by the AI
// extraction and seen in real profiles: "MM/YYYY - MM/YYYY", "MM/YYYY -
// Aktuell", "Monatsname[.] JJJJ–Monatsname[.] JJJJ", "Monatsname JJJJ–Heute",
// and a bare "JJJJ – JJJJ" (education).
const MONTH_NAME_TO_NUM = {
  jan: 1, januar: 1, january: 1,
  feb: 2, februar: 2, february: 2,
  mrz: 3, "märz": 3, maerz: 3, march: 3,
  apr: 4, april: 4,
  mai: 5, may: 5,
  jun: 6, juni: 6, june: 6,
  jul: 7, juli: 7, july: 7,
...
// Returns a single comparable number (year*12 + month) for one date token
// ("Feb. 2023", "01/2023", "2023"), or null if it can't be parsed at all.
function parsePeriodPoint(token) {
  const t = String(token || "").trim().toLowerCase();
  if (!t) return null;
  let m = t.match(/^(\d{1,2})\s*\/\s*(\d{4})$/); // "MM/YYYY"
  if (m) return Number(m[2]) * 12 + Number(m[1]);
  m = t.match(/^([a-zäöü]+)\.?\s+(\d{4})$/); // "Monatsname[.] YYYY"
  if (m && MONTH_NAME_TO_NUM[m[1]]) return Number(m[2]) * 12 + MONTH_NAME_TO_NUM[m[1]];
  m = t.match(/(\d{4})/); // fall back: any bare 4-digit year in the token
  if (m) return Number(m[1]) * 12 + 1;
  return null;
}

// Sort key = start-of-period. Unparseable periods sink to the very end
// (-Infinity) rather than crashing or randomly reordering the list.
function periodStartKey(period) {
  const startToken = String(period || "").split(/[-–—]/)[0];
  const key = parsePeriodPoint(startToken);
  return key === null ? -Infinity : key;
}

// Most recent/ongoing start date first; stable on ties (keeps original
// relative order for entries whose parsed start date is identical or
// unparseable) so this never shuffles things unnecessarily.
function sortByPeriodDesc(entries) {
  return entries
    .map((entry, index) => ({ entry, index, key: periodStartKey(entry.period) }))
...
function mergeAdditionalExperience(existingProfile, extracted) {
  ...
  // Merged in chronologically (most recent/ongoing first) rather than just
  // appended after the existing entries -- see sortByPeriodDesc above.
  profile.experience = sortByPeriodDesc([...profile.experience, ...added]);
  ...
  if (Array.isArray(src.education) && src.education.length) {
    ...
    profile.education = sortByPeriodDesc([...profile.education, ...addedEdu]);
  }
