...
function mergeAdditionalExperience(existingProfile, extracted) {
  ...
  // TEMP DIAGNOSTIC LOGGING: after the array-coercion fix, a real-world
  // attempt came back with no error but also no new positions added. Need to
  // see concretely whether candidates were extracted at all, and if so,
  // exactly which existing entry each one was matched against as a
  // "duplicate" -- to tell apart "dedup is correctly working, nothing new in
  // this upload" from "dedup is over-matching distinct positions". Safe to
  // remove once root-caused.
  console.log(`[experience-merge] candidates=${candidates.length}, existing=${profile.experience.length}`);
  candidates.forEach((e) => {
    console.log(`[experience-merge] candidate org="${e.org}" role="${e.role}" period="${e.period}"`);
  });
  profile.experience.forEach((e) => {
    console.log(`[experience-merge] existing org="${e.org}" role="${e.role}" period="${e.period}"`);
  });

  candidates.forEach((e, i) => {
    const dup = knownEntries.find((existing) => isDuplicateExperience(e, existing));
    if (dup) {
      console.log(`[experience-merge] SKIP "${e.org}" / "${e.role}" -- matched existing "${dup.org}" / "${dup.role}"`);
      skippedCount++;
      return;
    }
    console.log(`[experience-merge] ADD "${e.org}" / "${e.role}"`);
