 ...
 * for the very first CV import), this is additive: every entry the extractor
 * found is appended as a new experience item, however overlapping it may be
 * with what's already there — the account owner reviews/edits/removes
 * duplicates afterwards via the manual experience editor on /profile.
 */
function mergeAdditionalExperience(existingProfile, extracted) {
  ...
  const newEntries = Array.isArray(src.experience) ? src.experience.filter((e) => e && (e.org || e.role)) : [];
  ...
  if (Array.isArray(src.education) && src.education.length) {
    ...
    const newEdu = src.education
      .filter((e) => e && e.org)
      .map((e) => ({ org: e.org, role: e.role || null, period: e.period || null, note: e.note || null }));
    profile.education = [...profile.education, ...newEdu];
  }

  return { profile, addedCount: added.length };
}
