  ...
  input_schema: {
    ...
    properties: {
      ...
      suggestedQuestions: {
        ...
        minItems: 2,
        maxItems: 3,
        description: "2-3 kluge, konkrete Fragen, die man der Firma im Gespräch stellen kann — bezogen auf recherchierte Fakten und/oder die Stelle."
      }
    }
  }
};
...
async function researchCompany({ client, company, jobTitle, jobPostingText, language = "de" }) {
  const languageName = LANG_NAMES[normalizeLang(language)].de;
...
Decke ab, sofern auffindbar: Branche/Tätigkeit, ungefähre Grösse & Standort(e), wichtigste Produkte/Dienstleistungen, aktuelle News/Entwicklungen (letzte ca. 12 Monate). Schreibe in normalem Fliesstext auf ${languageName}.`;
...
async function structureInsights({ client, researchText, jobTitle, jobPostingText, language = "de" }) {
  const languageName = LANG_NAMES[normalizeLang(language)].de;
  const response = await client.messages.create({
    model: MODEL,
    max_tokens: 1200,
    system: `Du strukturierst einen bereits recherchierten Text in ein festes Format. Füge KEINE neuen Fakten über die Firma hinzu, die nicht im Recherchetext stehen — nur bei likelyTopics/suggestedQuestions darfst du sinnvolle Ableitungen/Empfehlungen basierend auf Stelle + Firmenprofil ergänzen. Schreibe alle Textfelder auf ${languageName}, unabhängig von der Sprache des Recherchetexts.`,
...
async function generateCompanyInsights({ company, jobTitle, jobPostingText, language = "de" }) {
  const client = getClient();
  const { researchText, sources } = await researchCompany({ client, company, jobTitle, jobPostingText, language });
  const structured = await structureInsights({ client, researchText, jobTitle, jobPostingText, language });
  return {
    ...structured,
    sources,
