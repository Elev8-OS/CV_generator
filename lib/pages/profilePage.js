...
<style>
${baseCss(accentColor)}
  header.top { background: var(--ink); color: #fff; padding: 0; }
  header.top .header-row-main { display: flex; justify-content: flex-start; align-items: center; padding-top: 12px; padding-bottom: 12px; }
  header.top h1 { font-size: 16px; margin: 0; font-weight: 700; white-space: nowrap; }
  .header-left { display: flex; align-items: center; gap: 12px; flex-wrap: wrap; }
  .header-divider { width: 1px; height: 20px; background: rgba(255,255,255,.16); flex: none; }
  .header-avatar {
    width: 36px; height: 36px; border-radius: 50%; flex: none; overflow: hidden;
    background: rgba(255,255,255,.14); border: 2px solid var(--accent);
    display: flex; align-items: center; justify-content: center; font-size: 13px; font-weight: 800; color: #fff;
  }
  .header-avatar img { width: 100%; height: 100%; object-fit: cover; }
  .header-name { font-size: 14px; font-weight: 700; color: #fff; white-space: nowrap; }
  .header-nav-bar { background: var(--accent); }
  .header-nav-inner { display: flex; align-items: center; justify-content: flex-end; gap: 16px; padding-top: 6px; padding-bottom: 6px; flex-wrap: wrap; }
  header.top .header-nav-bar a.navlink { font-size: 12.5px; color: rgba(255,255,255,.92); text-decoration: none; }
  header.top .header-nav-bar a.navlink:hover { color: #fff; text-decoration: underline; }
  header.top .header-nav-bar .lang-link { color: rgba(255,255,255,.75); }
  header.top .header-nav-bar .lang-link.active { color: #fff; background: rgba(255,255,255,.22); }
  @media (max-width: 700px) { header.top h1, .header-divider { display: none; } }
  @media (max-width: 480px) { .header-nav-inner { justify-content: center; } }
  main { padding: 22px 0 44px; }
  h2.title { font-size: 20px; margin: 0 0 6px; font-weight: 800; }
  p.subtitle { color: var(--grey); margin: 0 0 14px; line-height: 1.5; max-width: 720px; }
  ...
  .doc-row { display: flex; align-items: center; justify-content: space-between; padding: 12px 0; border-bottom: 1px solid var(--border); gap: 16px; flex-wrap: wrap; }
  .doc-row:last-child { border-bottom: none; }
  .doc-actions { display: flex; gap: 8px; align-items: center; flex: none; flex-wrap: wrap; }
  ...
  .banner-imported { background: #eaf5ff; border: 1px solid #bcdcf7; color: #1c4e73; padding: 12px 14px; border-radius: 10px; font-size: 13.5px; margin-bottom: 16px; }

  /* --- Tabs --- */
  .profile-tabs { display: flex; gap: 4px; margin-bottom: 16px; flex-wrap: wrap; border-bottom: 1.5px solid var(--border); }
  .profile-tab-btn {
    background: transparent; border: none; font-family: inherit; font-size: 14px; font-weight: 700; color: var(--grey);
    padding: 10px 16px; cursor: pointer; border-radius: 10px 10px 0 0; position: relative; transition: color .15s, background .15s;
  }
  ...
  .grid-cards { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 16px; align-items: start; }
  .grid-cards > .card + .card { margin-top: 0; }

  /* --- Experience editor --- */
  .exp-card {
    border: 1px solid var(--border); border-radius: 12px; padding: 16px; margin-top: 12px; background: var(--paper-soft);
  }
...
<body>
  <header class="top">
    <div class="container header-row-main">
      <div class="header-left">
        <h1>🛠️ ${esc(t(l, "common.appName"))}</h1>
        <span class="header-divider"></span>
        <div class="header-avatar">${avatarInner}</div>
        <span class="header-name">${esc(username || t(l, "profile.title"))}</span>
      </div>
    </div>
  ...
  <main class="container">
    ...
    <p class="subtitle" style="margin:14px 0 16px;">${esc(t(l, "profile.headerTagline"))}</p>
    ...
    <div class="profile-tab-panel" id="tab-profil">
      <div class="grid-cards">
        ...
        <div class="card">
          ...
          <div class="field-row">
            <div>
              <label>${esc(t(l, "profile.fieldAhv"))}</label>
              <input type="text" id="cAhv" placeholder="756.xxxx.xxxx.xx">
              <div class="subtitle" style="margin:4px 0 0;font-size:12px;">${esc(t(l, "profile.fieldAhvHint"))}</div>
            </div>
          </div>
          <div class="field-row">
        ...
        <div class="card">
          <h2 class="title" style="font-size:18px;">${esc(t(l, "profile.ravAdvisorTitle"))}</h2>
          <p class="subtitle">${esc(t(l, "profile.ravAdvisorSubtitle"))}</p>
          <div class="field-row">
            <div><label>${esc(t(l, "profile.fieldRavAdvisorName"))}</label><input type="text" id="ravAdvisorName"></div>
            <div><label>${esc(t(l, "profile.fieldRavAdvisorEmail"))}</label><input type="text" id="ravAdvisorEmail"></div>
          </div>
          <div class="field-row">
            <div><label>${esc(t(l, "profile.fieldRavAdvisorPhone"))}</label><input type="text" id="ravAdvisorPhone"></div>
            <div><label>${esc(t(l, "profile.fieldRavOffice"))}</label><input type="text" id="ravOffice"></div>
          </div>
          <div class="row">
            <button type="button" id="saveRavAdvisorBtn" class="btn btn-primary">${esc(t(l, "common.save"))}</button>
          </div>
          <div id="ravAdvisorStatus" class="status"></div>
        </div>
      </div>
    </div>
  ...
  <script>
    (function () {
      var LBL = {
        ...
        experienceSupplementDoneWithSkipped: ${JSON.stringify(t(l, "profile.experienceSupplementDoneWithSkipped"))},
        experienceSupplementNoNew: ${JSON.stringify(t(l, "profile.experienceSupplementNoNew"))},
        experienceSupplementNothingDetected: ${JSON.stringify(t(l, "profile.experienceSupplementNothingDetected"))},
      ...
      function populateContactFields() {
        ...
        document.getElementById('cAhv').value = personal.ahvNr || '';
      ...
      function syncContactIntoJson() {
        ...
        p.personal.ahvNr = document.getElementById('cAhv').value;
        var links = [];
      ...
      ['cName', 'cEmail', 'cPhone', 'cStreet', 'cZip', 'cCity', 'cLinkedin', 'cAhv'].forEach(function (id) {
      ...
      // ---------- RAV-Berater:in (advisor contact used only to prefill the
      // "An Berater:in senden" mailto on the dashboard — see
      // lib/pages/indexPage.js / public/app.js. Never printed on the RAV PDF
      // itself, see lib/pdf/rav.js.) ----------
      function populateRavAdvisorFields() {
        var p = getParsedProfile();
        if (!p) return;
        var rav = p.rav || {};
        document.getElementById('ravAdvisorName').value = rav.advisorName || '';
        document.getElementById('ravAdvisorEmail').value = rav.advisorEmail || '';
        document.getElementById('ravAdvisorPhone').value = rav.advisorPhone || '';
        document.getElementById('ravOffice').value = rav.office || '';
      }
      function syncRavAdvisorIntoJson() {
        var p = getParsedProfile();
        if (!p) return; // underlying JSON currently invalid — don't clobber it
        p.rav = p.rav || {};
        p.rav.advisorName = document.getElementById('ravAdvisorName').value;
        p.rav.advisorEmail = document.getElementById('ravAdvisorEmail').value;
        p.rav.advisorPhone = document.getElementById('ravAdvisorPhone').value;
        p.rav.office = document.getElementById('ravOffice').value;
        writeParsedProfile(p);
      }
      ['ravAdvisorName', 'ravAdvisorEmail', 'ravAdvisorPhone', 'ravOffice'].forEach(function (id) {
        document.getElementById(id).addEventListener('blur', syncRavAdvisorIntoJson);
      });
      populateRavAdvisorFields();

      document.getElementById('saveRavAdvisorBtn').addEventListener('click', async function () {
        syncRavAdvisorIntoJson();
        var status = document.getElementById('ravAdvisorStatus');
        var p = getParsedProfile();
        if (!p) { status.textContent = LBL.invalidJson; status.className = 'status err'; return; }
        status.textContent = '…';
        status.className = 'status';
      ...
      function buildExpCard(entry, idx) {
        ...
        bulletsArea.rows = 4;
        bulletsArea.value = Array.isArray(entry.bullets) ? entry.bullets.join(String.fromCharCode(92) + 'n') : '';
        bulletsArea.addEventListener('input', function () {
          entry.bullets = bulletsArea.value.split(String.fromCharCode(92) + 'n').map(function (s) { return s.trim(); }).filter(Boolean);
        });
          ...
          if (res.ok) {
            ...
            if (data.addedCount > 0) {
              status.textContent = skipped > 0
                ? LBL.experienceSupplementDoneWithSkipped.replace('{{n}}', data.addedCount).replace('{{s}}', skipped)
                : LBL.experienceSupplementDone.replace('{{n}}', data.addedCount);
              status.className = 'status ok';
              window.location.hash = 'erfahrung';
              setTimeout(function () { window.location.reload(); }, 900);
            } else if (skipped > 0) {
              // Positions were recognized but every one already matches an existing entry.
              status.textContent = LBL.experienceSupplementNoNew;
              status.className = 'status';
            } else {
              // Nothing was recognized at all — likely a parsing/format issue, not "already known".
              status.textContent = LBL.experienceSupplementNothingDetected;
              status.className = 'status err';
            }
          } else {
