...
const { accentLight } = require("../accentColor");
...
function renderAppPage({ profile, generated, entry, libraryDocs = [], photo, lang, accentColor }) {
  const l = normalizeLang(lang || entry.language);
  const heroPillText = accentLight(accentColor);
  ...
  const docs = [
    { href: `/pdf/${entry.slug}/cv`, label: t(l, "appPage.docCvLabel"), sub: t(l, "appPage.docCvSub") },
    { href: `/pdf/${entry.slug}/cover`, label: t(l, "appPage.docCoverLabel"), sub: t(l, "appPage.docCoverSub") },
    ...libraryDocs.map((d) => ({
      href: `/documents/library/${d.id}`,
      label: d.title,
      sub: CATEGORY_BY_KEY[d.category] ? t(l, `docCategory.${d.category}`) : t(l, "appPage.docGenericSub")
    }))
  ];

  const extraLinksHtml = (profile.links || [])
    .filter((link) => link && link.url)
    .map((link) => `<a href="${esc(linkedinHref(link.url))}" target="_blank" rel="noopener">🔗 ${esc(link.label || link.url)}</a>`)
    .join("");
...
<title>${esc(p.name)} — ${g.jobTitle ? esc(g.jobTitle) : esc(t(l, "appPage.pillPlain"))}${g.company ? " · " + esc(g.company) : ""}</title>
...
${baseCss(accentColor)}
  ...
  .hero::after {
    ...
    background: radial-gradient(500px 300px at 90% 10%, rgba(var(--accent-rgb),.35), transparent 70%);
  }
  .hero .container { position: relative; z-index: 1; }
  .hero .pill { background: rgba(var(--accent-rgb),.18); color: ${heroPillText}; }
...
<body>
  <nav class="sticky-bar" id="stickyBar">
    <div class="container">
      <div class="who">${esc(p.name)} <span>· ${esc(g.jobTitle || t(l, "appPage.pillPlain"))}</span></div>
      <div class="actions">
        <a class="btn btn-primary" href="/pdf/${entry.slug}/cv" target="_blank" rel="noopener">${esc(t(l, "appPage.navCv"))}</a>
        <a class="btn btn-primary" href="/pdf/${entry.slug}/cover" target="_blank" rel="noopener">${esc(t(l, "appPage.navCover"))}</a>
      </div>
    </div>
  </nav>
  ...
  <div class="hero">
    <div class="container">
      <div class="hero-flex fade-up">
        <div class="hero-text">
          <span class="pill">${g.company ? esc(t(l, "appPage.pillFor", { company: g.company })) : esc(t(l, "appPage.pillPlain"))}</span>
          <h1>${esc(p.name)}</h1>
          <div class="headline">${esc(g.headline)}${g.jobTitle ? " · " + esc(g.jobTitle) : ""}</div>
          <p class="intro">${esc(g.digitalPageIntro)}</p>
          <div class="cta-row">
            <a class="btn btn-primary" href="/pdf/${entry.slug}/cv" target="_blank" rel="noopener">${esc(t(l, "appPage.downloadCv"))}</a>
            <a class="btn btn-primary" href="/pdf/${entry.slug}/cover" target="_blank" rel="noopener">${esc(t(l, "appPage.downloadCover"))}</a>
          </div>
          <div class="contact">
            <a href="mailto:${esc(p.email)}">✉ ${esc(p.email)}</a>
            <a href="tel:${esc(telHref(p.telefon))}">☎ ${esc(p.telefon)}</a>
            <a href="${mapsHref}" target="_blank" rel="noopener">📍 ${esc(p.plz)} ${esc(p.ort)}</a>
            ${p.linkedin ? `<a href="${esc(linkedinHref(p.linkedin))}" target="_blank" rel="noopener">🔗 ${esc(p.linkedin)}</a>` : ""}
            ${extraLinksHtml}
          </div>
        </div>
        ${photo && photo.dataUri ? `<div class="hero-photo"><img src="${photo.dataUri}" alt="${esc(p.name)}"></div>` : ""}
      </div>
    </div>
  </div>
  ...
  <main class="container">
    <section class="block reveal">
      <div class="section-title">${esc(t(l, "appPage.sectionProfile"))}</div>
      <div class="profile-text">${esc(g.summary)}</div>
    </section>

    <section class="block reveal">
      <div class="section-title">${esc(t(l, "appPage.sectionWhy"))}</div>
      <div class="why-card">${nl2p(g.whyThisRole)}</div>
    </section>

    <section class="block reveal">
      <div class="section-title">${esc(t(l, "appPage.sectionStrengths"))}</div>
      <div class="strengths-grid">${strengthsHtml}</div>
    </section>

    <section class="block reveal">
      <div class="section-title">${esc(t(l, "appPage.sectionExperience"))}</div>
    ...
    ${
      quote
        ? `
    <section class="block reveal">
      <div class="section-title">${esc(t(l, "appPage.sectionReference"))}</div>
    ...
    <section class="block reveal">
      <div class="two-col">
        <div>
          <div class="section-title">${esc(t(l, "appPage.sectionLanguages"))}</div>
          <div class="kv-list">
            ${profile.languages.map((lg) => `<div><b>${esc(lg.name)}</b><span class="grey">${esc(lg.level)}</span></div>`).join("")}
          </div>
        </div>
        <div>
          <div class="section-title">${esc(t(l, "appPage.sectionEducation"))}</div>
          <div class="kv-list">
            ${
              primaryEducation
                ? `<div><b>${esc(primaryEducation.title || "")}</b><span class="grey">${esc(primaryEducation.note || primaryEducation.period || "")}</span></div>${
                    primaryEducation.org ? `<div><b>${esc(primaryEducation.org)}</b><span class="grey">${esc(primaryEducation.period || "")}</span></div>` : ""
                  }`
                : ""
            }
    ...
    <section class="block reveal">
      <div class="section-title">${esc(t(l, "appPage.sectionDocs"))}</div>
      <div class="docs-grid">
        ${docs
          .map(
            (d) => `
    ...
    <div class="transparency reveal">
      <div class="tag">✦ ${esc(t(l, "appPage.transparencyTag"))}</div>
      <p>${esc(t(l, "appPage.transparencyText"))}</p>
    </div>

    <footer>
      ${esc(t(l, "appPage.footerCreatedOn", { date: new Date(entry.createdAt).toLocaleDateString(localeFor(l)) }))} · ${esc(p.name)}
    </footer>
  </main>
