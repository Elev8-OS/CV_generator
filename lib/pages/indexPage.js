...
// Per-application hidden panel row (toggled via the row's "RAV-Angaben"
// button) holding the fields lib/pdf/rav.js needs that aren't captured
// anywhere else — see lib/store.js updateApplicationRav for the field list.
// Hidden by default via a CSS class (toggled in public/app.js) rather than
// inline style, so the status-filter handler can also close it cleanly.
function ravPanelRow(a, l) {
  const opt = (value, current, label) => `<option value="${value}" ${value === current ? "selected" : ""}>${esc(label)}</option>`;
  return `
      <tr class="rav-panel-row" data-rav-panel="${esc(a.slug)}">
        <td colspan="4">
          <div class="rav-panel">
            <div class="rav-checkbox-row">
              <input type="checkbox" id="rav-assign-${esc(a.slug)}" data-rav-assignment="${esc(a.slug)}" ${a.ravAssignment ? "checked" : ""}>
              <label for="rav-assign-${esc(a.slug)}">${esc(t(l, "rav.fieldRavAssignment"))}</label>
            </div>
            <div class="rav-field-row">
              <div>
                <label>${esc(t(l, "rav.fieldPensum"))}</label>
                <select data-rav-pensum-type="${esc(a.slug)}">
                  ${opt("vollzeit", a.pensumType || "vollzeit", t(l, "rav.optionVollzeit"))}
                  ${opt("teilzeit", a.pensumType || "vollzeit", t(l, "rav.optionTeilzeit"))}
                </select>
              </div>
              <div>
                <label>${esc(t(l, "rav.pensumPercentPlaceholder"))}</label>
                <input type="number" min="1" max="100" placeholder="%" data-rav-pensum-percent="${esc(a.slug)}" value="${a.pensumPercent || ""}">
              </div>
              <div>
                <label>${esc(t(l, "rav.fieldBewerbungsart"))}</label>
                <select data-rav-art="${esc(a.slug)}">
                  <option value="">—</option>
                  ${opt("brieflich_elektronisch", a.bewerbungsart || "", t(l, "rav.optionBrieflich"))}
                  ${opt("persoenlich", a.bewerbungsart || "", t(l, "rav.optionPersoenlich"))}
                  ${opt("telefonisch", a.bewerbungsart || "", t(l, "rav.optionTelefonisch"))}
                </select>
              </div>
            </div>
            <div class="rav-field-row">
              <div>
                <label>${esc(t(l, "rav.fieldCompanyAddress"))}</label>
                <input type="text" data-rav-address="${esc(a.slug)}" value="${esc(a.companyAddress || "")}">
              </div>
              <div>
                <label>${esc(t(l, "rav.fieldContactPhone"))}</label>
                <input type="text" data-rav-phone="${esc(a.slug)}" value="${esc(a.contactPhone || "")}">
              </div>
            </div>
            <div class="rav-field-row">
              <div>
                <label>${esc(t(l, "rav.fieldAbsagegrund"))}</label>
                <input type="text" data-rav-reason="${esc(a.slug)}" value="${esc(a.absagegrund || "")}">
              </div>
            </div>
            <div class="row">
              <button type="button" class="btn btn-primary" data-rav-save="${esc(a.slug)}">${esc(t(l, "common.save"))}</button>
              <span class="status" data-rav-status="${esc(a.slug)}"></span>
            </div>
          </div>
        </td>
      </tr>`;
...
function renderIndexPage({
    ...
    .map((a) => {
      ...
      <tr id="row-${a.slug}" data-row="${a.slug}" data-status="${esc(a.status || "")}">
        ...
        a.publicDisabled ? esc(t(l, "dashboard.activatePageTitle")) : esc(t(l, "dashboard.deactivatePageTitle"))
          ...
          ${
            a.status !== "entwurf"
              ? `<button class="link" data-rav-toggle="${a.slug}">${esc(t(l, "rav.editBtn"))}</button>`
              : ""
          }
          <button class="link danger" data-delete="${a.slug}">${esc(t(l, "dashboard.deleteBtn"))}</button>
        </td>
      </tr>
      ${a.status !== "entwurf" ? ravPanelRow(a, l) : ""}`;
    })
    .join("");
  ...
  const appI18n = {
    ...
    // Dynamic, not a fixed guess — see lib/timings.js / server.js.
    generating: t(l, "dashboard.generating", { seconds: avgGenerateSeconds }),
    ...
    insightsResearching: t(l, "dashboard.insightsResearching", { seconds: avgInsightsSeconds }),
    ...
    ravSaveOk: t(l, "rav.saveOk"),
    ravSaveError: t(l, "rav.saveError"),
    applicantName,
    ravAdvisorName,
    ravAdvisorEmail,
    // %MONTH%/%NAME%/%FILENAME% are literal tokens in these dictionary
    // strings, substituted client-side in public/app.js (buildRavMailto) —
    // not lib/i18n.js's own {{var}} interpolation syntax.
    ravMailtoSubjectTemplate: t(l, "rav.mailtoSubjectTemplate"),
    ravMailtoGreetingWithName: t(l, "rav.mailtoGreetingWithName"),
    ravMailtoGreetingGeneric: t(l, "rav.mailtoGreetingGeneric"),
    ravMailtoBodyTemplate: t(l, "rav.mailtoBodyTemplate"),
    ravMailtoClosing: t(l, "rav.mailtoClosing")
  };
...
${baseCss(accentColor)}
  ...
  .rav-panel-row { display: none; }
  .rav-panel-row.rav-open { display: table-row; }
  .rav-panel-row td { background: var(--paper-soft); }
  .rav-panel { padding: 6px 4px 10px; max-width: 640px; }
  .rav-field-row { display: flex; gap: 12px; flex-wrap: wrap; margin-top: 10px; }
  .rav-field-row > div { flex: 1 1 160px; }
  .rav-field-row label, .rav-checkbox-row label { display: block; font-size: 12px; font-weight: 600; margin: 0 0 5px; color: var(--ink-soft); }
  .rav-checkbox-row { display: flex; align-items: center; gap: 8px; margin-top: 2px; }
  .rav-checkbox-row input[type=checkbox] { width: auto; accent-color: var(--accent); cursor: pointer; }
  .rav-checkbox-row label { margin: 0; cursor: pointer; }
  .rav-panel select, .rav-panel input[type=text], .rav-panel input[type=number] { width: 100%; }
  .rav-panel .row { display: flex; gap: 10px; margin-top: 12px; align-items: center; }
  .rav-download-row { display: flex; gap: 10px; align-items: center; flex-wrap: wrap; }
  .rav-download-row input[type=month] { font-family: inherit; font-size: 14px; padding: 10px 12px; border-radius: 10px; border: 1.5px solid var(--border); background: #fff; color: var(--ink); }
...
<body>
  ...
  <main class="container">
    ...
    <div class="card rav-card">
      <h2 class="title" style="font-size:16px;">${esc(t(l, "rav.sectionTitle"))}</h2>
      <p class="subtitle" style="margin-bottom:14px;">${esc(t(l, "rav.sectionSubtitle"))}</p>
      ${!ahvNr ? `<div class="warning">${esc(t(l, "rav.ahvMissingHint"))}</div>` : ""}
      ${!ravAdvisorEmail ? `<div class="warning">${esc(t(l, "rav.advisorMissingHint"))}</div>` : ""}
      <div class="rav-download-row">
        <label style="margin:0;white-space:nowrap;">${esc(t(l, "rav.monthLabel"))}</label>
        <input type="month" id="ravMonth" value="${esc(currentMonthValue)}">
        <button type="button" id="ravDownloadBtn" class="btn btn-primary">${esc(t(l, "rav.downloadBtn"))}</button>
        ${ravAdvisorEmail ? `<button type="button" id="ravSendBtn" class="btn btn-outline">${esc(t(l, "rav.sendToAdvisorBtn"))}</button>` : ""}
      </div>
      ${ravAdvisorEmail ? `<p class="subtitle" style="margin:8px 0 0;font-size:12px;">${esc(t(l, "rav.sendToAdvisorHint"))}</p>` : ""}
    </div>
