...
function renderIndexPage({ applications, hasApiKey, statuses, baseUrl, savedSearches = [], username = "" }) {
  ...
  const rows = applications
    .map((a) => {
      ...
      const followupMailto = buildFollowUpMailto({ generated: a.generated, profile: a.profileSnapshot, entry: a });
      ...
      <tr id="row-${a.slug}" data-row="${a.slug}" data-status="${esc(a.status || "")}">
        <td>
          ...
          ${
            dupTarget
              ? `<a class="dup-badge" href="#row-${esc(a.duplicateOfSlug)}" title="Du hast bei ${esc(a.company)} bereits am ${esc(new Date(dupTarget.createdAt).toLocaleDateString("de-CH"))} beworben (Status: ${esc((statusByKey[dupTarget.status] || {}).label || dupTarget.status)}) — bitte prüfen, ob das gewollt war.">⚠️ Mögliche Dublette</a>`
              : ""
          }
        </td>
        ...
        <td class="actions">
          ...
          ${
            canFollowUp
              ? `<a href="${esc(followupMailto)}" class="link" data-followup="${a.slug}" title="Höfliche Nachfrage-Mail vorbereiten — Status wird danach automatisch auf 'Follow-up gemacht' gesetzt">🔔 Nachfassen</a>`
              : ""
          }
          ${
            canPrepInterview
              ? insightsReady
                ? `<a href="/pdf/${a.slug}/insights" target="_blank" class="link" title="Automatisch recherchiertes Briefing zur Gesprächsvorbereitung">🏢 Firmen-Insights</a>
             <a href="#" data-regen-insights="${a.slug}" class="link" style="font-size:11px;" title="Neu recherchieren (z.B. falls es neue Firmen-News gibt)">↻</a>`
                : `<a href="#" data-gen-insights="${a.slug}" class="link" title="Recherchiert die Firma automatisch im Web und erstellt ein PDF-Briefing zur Gesprächsvorbereitung">🏢 Firmen-Insights erstellen</a>`
              : ""
          }
...
<title>Bewerbungs-Generator${username ? " — " + esc(username) : ""}</title>
  ...
  <header class="top">
    <div class="container">
      ...
      <span style="display:flex;align-items:center;gap:16px;">
        ${username ? `<span class="navlink">Eingeloggt als <b style="color:#fff;">${esc(username)}</b></span>` : ""}
        <a class="navlink" href="/profile">Profil-Daten bearbeiten →</a>
        <a class="navlink" href="#" id="logoutLink">Logout</a>
      </span>
    </div>
  </header>
      ...
      <div class="card">
        ...
        <table>
          <thead>
            <tr>
              <th>Bewerbung</th>
              <th title="Ehrliche KI-Einschätzung, wie gut dein Profil zu dieser Stelle passt">Fit</th>
              <th>Status</th>
              <th>Aktionen</th>
            </tr>
          </thead>
