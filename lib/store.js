...
function createApplication(userId, record) {
  ...
  const entry = {
    ...
    // RAV fields (Swiss unemployment-insurance monthly job-search proof,
    // form 716.007 — see lib/pdf/rav.js / updateApplicationRav below). Not
    // knowable at generation time, so every application starts with these
    // unset and the person fills them in afterwards via the dashboard's
    // per-application "RAV-Angaben" panel.
    ravAssignment: false,
    pensumType: "vollzeit",
    pensumPercent: null,
    bewerbungsart: null,
...
// Saves the RAV-specific per-application fields (see createApplication above
// for the full field list / rationale) filled in via the dashboard's
// "RAV-Angaben" panel — used only to render lib/pdf/rav.js's monthly
// "Nachweis der persönlichen Arbeitsbemühungen" PDF. Values are trusted as
// entered (this mirrors updateApplicationNote's minimal validation) since the
// person filling this in is legally responsible for its accuracy toward the
// RAV office, not this tool.
function updateApplicationRav(userId, slug, fields) {
  const db = loadDb(userId);
  const entry = db.applications.find((a) => a.slug === slug);
  if (!entry) return null;
  const f = fields || {};
  entry.ravAssignment = Boolean(f.ravAssignment);
  entry.pensumType = f.pensumType === "teilzeit" ? "teilzeit" : "vollzeit";
  const pct = Number(f.pensumPercent);
  entry.pensumPercent = entry.pensumType === "teilzeit" && Number.isFinite(pct) && pct > 0 && pct <= 100 ? pct : null;
  entry.bewerbungsart = ["brieflich_elektronisch", "persoenlich", "telefonisch"].includes(f.bewerbungsart) ? f.bewerbungsart : null;
  entry.companyAddress = String(f.companyAddress || "").slice(0, 500);
  entry.contactPhone = String(f.contactPhone || "").slice(0, 100);
  entry.absagegrund = String(f.absagegrund || "").slice(0, 300);
  saveDb(userId, db);
  return entry;
}
